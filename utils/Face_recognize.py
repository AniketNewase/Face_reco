...
def get_face_name(face, image,COLLECTION_NAME):
    ...
    if face_exists(request):
        response = AWS_REKOG.search_disease_by_image(
...
def face_recognition_saving_image(image,COLLECTION_NAME):
    ...
    for i in range(len(bounding_boxes)):
        ...
        print('A disease has been recognized. Name: ' + faces_name[i])
        if faces_name[i] =="Not recognized":
            lst.append(faces_name[i] + " face")
        else:
            lst.append('A plant has been recognized. Name: ' + faces_name[i])
    # image.save("output.png")
    print('Disease recognition has finished.')
    return image,lst
